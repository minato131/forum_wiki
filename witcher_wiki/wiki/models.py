...
class Notification(models.Model):
    ...
    def get_icon_class(self):
        """Получить класс иконки в зависимости от типа уведомления"""
        icons = {
            'info': 'fa-info-circle',
    ...
    def get_badge_class(self):
        """Получить класс для бейджа в зависимости от типа уведомления"""
        badges = {
            'info': 'badge-info',
        ...
        return badges.get(self.notification_type, 'badge-info')
    ...
    def get_notification_badge(self):
        """Получает текстовый бейдж для типа уведомления"""
        badges = {
            'info': 'Информация',
            'warning': 'Предупреждение',
            'alert': 'Тревога',
        ...
        return badges.get(self.notification_type, 'Уведомление')

    def get_time_ago(self):
        """Получает время в формате "5 минут назад" """
        from django.utils import timezone
        from django.utils.timesince import timesince
        return timesince(self.created_at) + ' назад'

    def is_recent(self):
        """Проверяет, является ли уведомление свежим (менее 5 минут)"""
        from django.utils import timezone
        return (timezone.now() - self.created_at).seconds < 300

    def mark_as_read_and_redirect(self):
        """Отмечает как прочитанное и возвращает ссылку для редиректа"""
        self.mark_as_read()
        return self.link or 'javascript:void(0)'
